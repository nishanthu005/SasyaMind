/**
 * App.js — updated to include OfflineBanner
 * Drop this in src/App.js (replaces your existing file)
 */

import React, { useState, useEffect } from 'react';
import Sidebar from './components/Sidebar';
import OfflineBanner from './components/OfflineBanner';
import Dashboard from './pages/Dashboard';
import DiseaseDetection from './pages/DiseaseDetection';
import YieldPrediction from './pages/YieldPrediction';
import Irrigation from './pages/Irrigation';
import Fertilizer from './pages/Fertilizer';
import { LanguageProvider } from './context/LanguageContext';
import { LocationProvider } from './context/LocationContext';
import './App.css';

function AppInner() {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [sensorData, setSensorData] = useState({ temp: 28.4, moisture: 62 });

  // Read ?tab=xxx from URL (used by PWA shortcuts in manifest.json)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const tab = params.get('tab');
    if (tab) setActiveTab(tab);
  }, []);

  useEffect(() => {
    const interval = setInterval(() => {
      setSensorData({
        temp:     +(28 + Math.sin(Date.now() / 5000) * 2).toFixed(1),
        moisture: Math.floor(60 + Math.cos(Date.now() / 7000) * 8),
      });
    }, 2000);
    return () => clearInterval(interval);
  }, []);

  const pages = {
    dashboard: Dashboard,
    disease:   DiseaseDetection,
    yield:     YieldPrediction,
    irrigation: Irrigation,
    fertilizer: Fertilizer,
  };
  const PageComponent = pages[activeTab] || Dashboard;

  return (
    <div className="app">
      {/* OfflineBanner sits above everything and is sticky */}
      <OfflineBanner />

      <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} sensorData={sensorData} />
      <main className="main-content">
        <PageComponent />
      </main>
    </div>
  );
}

export default function App() {
  return (
    <LanguageProvider>
      <LocationProvider>
        <AppInner />
      </LocationProvider>
    </LanguageProvider>
  );
}
